...
public class PrintUtil {
    ...
    public static void print(Activity activity, int totalPage) {
        metricsListener = null;
        ...
        if (needHelpToInstallPlugin(activity)) {
            ...
            readyToPrint(activity, totalPage);
        }
    }
    ...
    public static void readyToPrint(Activity activity, int totalPage) {
        if (printJobData == null) {
            Log.e(TAG, "Please set PrintJobData first");
            Toast.makeText(activity.getApplicationContext(), TAG + ": " + R.string.set_print_job_data, Toast.LENGTH_LONG).show();
            return;
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP || printJobData.containsPDFItem()) {
            createPrintJob(activity, totalPage);
        } else {
            startPrintPreviewActivity(activity);
        }
    }
    ...
    public static void createPrintJob(final Activity activity, int totalPage) {
        PrintManager printManager = (PrintManager) activity.getSystemService(Context.PRINT_SERVICE);
        PrintDocumentAdapter adapter = new HPPrintDocumentAdapter(activity, printJobData, false, totalPage);
